...
object WebScraperGenericArticleTests extends TestSuite with TestUtils {
  
  var html = ""

  val GenericURL = "http://jdiaz.me"/** Replace with URL */
  /** BEGIN MANUAL SECTION */
  /** END MANUAL SECTION */

  override def utestBeforeEach(path: Seq[String]): Unit = {
    if (html.isEmpty) {
      val htmlStream = getClass.getResourceAsStream("/Generic.html.test")
      html = Source.fromInputStream(htmlStream).getLines.mkString
    }
  }

  def tests = Tests {
    test("Web Scrape Generic") {
      /** END BEGIN SECTION */
      val page = WebPage(GenericURL, CountryType.EEUU, SourceType.TEST)
      val linkURL = "/* Replace me */"
      val linkText = "/* Replace me */"
      val author = "/* Replace me */"
      val maybeArticle: Option[WebArticle] = WebScraperGenericArticle(
        page,
        linkText,
        linkURL,
        html,
  ...
  private def testArticle(
    ...
    assert(!checkOptionalMatch(a.datePublished, ""))
